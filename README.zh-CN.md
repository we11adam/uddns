...
升级器会精确选择当前平台对应的 GoReleaser 资产并通过 HTTPS 下载。它先用运行中
二进制内置的发布公钥验证 `checksums.txt.minisig`，通过后才根据 `checksums.txt`
校验归档的 SHA-256；随后严格验证归档只包含一个可执行文件，并确认暂存二进制报告的
版本。最后它会在原文件旁创建 `.previous` 备份，并以原子操作替换已安装的可执行
文件。自升级会拒绝未签名的历史版本。解析后的目标必须是普通文件。由于操作系统可能
在 UDDNS 检查启动路径前就解析符号链接，升级器无法可靠识别所有由包管理器管理的
安装；这类情况请改用对应包管理器的升级命令。
...
- `updater`：要使用的 updater，例如 `cloudflare`、`aliyun`、`duckdns`、`lightdns`
  或 `scaleway`。
- `record`：需要更新的 DNS 记录。DuckDNS 使用不包含 `.duckdns.org` 的子域名。
- `zone`：Cloudflare、Aliyun 和 Scaleway 可选的 DNS zone 覆盖。
...
- `updater_api`：强制通过所选 updater 对应的 DNS 服务商 API 查询当前记录。
  Cloudflare、Aliyun 和 Scaleway 支持该模式。DuckDNS 和 LightDNS 不支持，所以与
  `verify: updater_api` 一起使用时 `config check` 会失败。
...
维护者发布版本前应遵循[发布签名操作手册](docs/release-signing.md)。
