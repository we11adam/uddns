...
- 兼容 systemd credential 使用的 `root:root`、`0440` ACL 表示，同时保持普通配置
  文件原有的严格私有权限检查。
- 生成的 systemd 服务会在启动前验证 credential 和配置，同步报告进程准备阶段的
  错误，并在安装失败时恢复原可执行文件、unit 及服务状态。

## v1.9.0 - 2026-07-25

### 新增

- 新增 `uddns version`，支持可读文本和 JSON 输出。
- 所有发布目标均支持稳定版检查，Linux、macOS 和 FreeBSD 另外支持原子自升级
  与回滚。升级过程会精确匹配平台资产、校验 SHA-256、严格验证单文件归档、
  检查暂存二进制版本与平台，并在原文件旁保留 `.previous` 备份。
...
- 修复 RouterOS provider 的 PPPoE 地址选择，忽略已禁用的 IPv4 和 IPv6 地址。
- 为阿里云 DNS 记录分页增加上限，避免异常的连续满页响应导致请求无法结束或返回部分结果。
- Cloudflare 遇到无效标识符或资源不存在响应时会刷新过期的 zone ID，并针对删除后重建的 zone 进行一次定向重试。

## v1.7.1 - 2026-07-13

### 修复

- 修复发布归档，使其仅包含对应平台的可执行文件，恢复与安装器严格单文件归档校验的兼容性。
